...
console.log(`# アンケート集計 — 応募 ${data.length} 件 / 除外 ${excluded.length} 件(規則該当) / 集計対象 n=${n}`);
...
console.log("\n※限定句: 自主スクリーニング調査(リサーチパネル回答者)。自己選択・自己申告・謝礼動機の三重バイアスあり。全事業者の割合ではない。支払意向は自己申告であり購買実績ではない。");
