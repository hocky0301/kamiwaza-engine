 ...
 * 一方、個別プロパティの assert では元の const 型のまま参照する（キーが消えたら
 * ランタイムではなく `tsc --noEmit` の時点で落ちてほしいため）。
 * ==========================================================================*/
...
const SCHEMA = ANALYZE_OUTPUT_JSON_SCHEMA as unknown as SchemaNode;
 ...
 */
function walkSchema(root: SchemaNode, isTarget: (n: SchemaNode) => boolean): WalkHit[] {
  const hits: WalkHit[] = [];
...
function validate(schema: SchemaNode, data: unknown, path = "$"): ValidationError[] {
...
const validateOutput = (data: unknown): ValidationError[] => validate(SCHEMA, data);
